...
AIOS 是一个通用 AI 操作系统原型，演示 AI 如何像管理量子资源一样管理经典系统资源。

### ✅ 已实现功能

| 模块 | 功能 | 演示效果 |
|------|------|----------|
| AI 进程调度器 | 规则 + 启发式分类，检测问题进程 | 识别 Chrome、Photoshop、虚拟机为问题进程 |
| AI 内存管理 | 内存压力检测、泄漏预警 | 持续监控内存使用 |
| AI 磁盘 I/O 管理 ✨ | I/O 限流、队列拥塞控制 | 多次触发 I/O 限流 |
| AI 网络管理 ✨ | 带宽限制、拥塞控制 | 触发网络限速 |
| AI 健康检查 ✨ | 进程无响应检测、资源滥用终止 | 终止了虚拟机和 Photoshop |
...
├── AIOS/
│   ├── Program.cs              # 主程序入口
│   ├── AIModules.cs            # 基础 AI 调度器和内存管理器
│   ├── AIEnhancedModules.cs    # ML 分类器、磁盘/网络/健康检查器
│   └── AIOS.csproj
```
